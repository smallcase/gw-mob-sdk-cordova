...
- (void)setCordovaSdkVersion:(CDVInvokedUrlCommand*)command {
    ...
    if ([SmallcaseGateway.shared respondsToSelector:@selector(setSDKTypeWithType:)]) {
        [SmallcaseGateway.shared setSDKTypeWithType:@"cordova"];
    }
    
    NSString *versionString = [command.arguments objectAtIndex:0];
    if ([SmallcaseGateway.shared respondsToSelector:@selector(setHybridSDKVersionWithVersion:)]) {
        [SmallcaseGateway.shared setHybridSDKVersionWithVersion:versionString];
    }
}
...
- (void)getSdkVersion:(CDVInvokedUrlCommand*)command {
    ...
    if ([SmallcaseGateway.shared respondsToSelector:@selector(getSdkVersion)]) {
        nativeSdkString = [NSString stringWithFormat:@"ios:%@", [SmallcaseGateway.shared getSdkVersion]];
    }
...
- (void)isUserConnected:(CDVInvokedUrlCommand *)command {
    ...
    if ([SmallcaseGateway.shared respondsToSelector:@selector(isUserConnected)]) {
        isUserConnected = [SmallcaseGateway.shared isUserConnected];
    }
...
- (void)triggerTransaction:(CDVInvokedUrlCommand*)command {
    ...
    [SmallcaseGateway.shared triggerTransactionFlowWithTransactionId:transactionId 
...
- (void)setConfigEnvironment:(CDVInvokedUrlCommand*)command {
    ...
    [SmallcaseGateway.shared setupWithConfig:config completion:^(BOOL success, NSError *error) {
        if (success) {
...
- (void)initSCGateway:(CDVInvokedUrlCommand*)command {
    ...
    [SmallcaseGateway.shared initializeGatewayWithSdkToken:authToken completion:^(BOOL success, NSError *error) {
        if (success) {
            NSMutableDictionary *responseDict = [[NSMutableDictionary alloc] init];
...
- (void)triggerLeadGen:(CDVInvokedUrlCommand*)command {
    ...
    if ([SmallcaseGateway.shared respondsToSelector:@selector(triggerLeadGenWithPresentingController:params:completion:)]) {
        [SmallcaseGateway.shared triggerLeadGenWithPresentingController:presentingController 
                                                           params:userDetails 
...
- (void)logout:(CDVInvokedUrlCommand*)command {
    ...
    [SmallcaseGateway.shared logoutUserWithPresentingController:presentingController completion:^(BOOL success, NSError *error) {
        if (success) {
            pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK];
        } else {
            NSMutableDictionary *responseDict = [[NSMutableDictionary alloc] init];
...
- (void)showOrders:(CDVInvokedUrlCommand *)command {
    ...
    [SmallcaseGateway.shared showOrdersWithPresentingController:presentingController completion:^(BOOL success, NSError *error) {
        if (success) {
            pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK];
        } else {
            NSMutableDictionary *responseDict = [[NSMutableDictionary alloc] init];
